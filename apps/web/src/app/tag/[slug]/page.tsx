...
async function fetchStories(slug: string): Promise<StoriesPage> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || ""}/backend/stories?tags=${encodeURIComponent(slug)}`, {
    next: { revalidate },
  });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4">
      <header className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-semibold text-gray-900">Tag: {params.slug}</h1>
        <SearchBox />
      </header>
      <Filters />
      <div className="mt-2 space-y-3">
        {items.length === 0 ? (
          <EmptyState message="No stories found for this tag." />
        ) : (
